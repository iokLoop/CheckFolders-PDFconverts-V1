...
class FunctionsClass():
	...
	def ConvertToPDF(self,_dirPDF,_dirIMAGICK):
		...
		for filename in ListName:
			...
			if  not (file_extension == ".PDF" or file_extension == ".pdf"):
...
#### BORRAR ESTE SEGMENTO >
			print(filename)
			n+=1
			_PDFname=_PDFname + str(n)
			_PDFuserName=_PDFuserName + str(n)
			_PDFuserLast=_PDFuserLast + str(n)
			_PDFuserNum=_PDFuserNum + str(n)
			_PDFfileType=_PDFfileType + str(n)
#### BORRAR ESTE SEGMENTO <
