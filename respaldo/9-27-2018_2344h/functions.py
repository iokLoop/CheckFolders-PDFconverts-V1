...
class Functions():
	...
	def DirChangeDetect(self):
		...
		print ("el archivo before es : \n",_LObjBE)
		print("el archivo after es : \n",_LObjAF)
	...
	def ConvertToPDF(self,_dirPDF,_dirIMAGICK,_fileName,_fileName2,_fileName3,_fileName4):
		try:
			...
			#os.chdir(_dirPDF)
			print("el directorio en el que se va a ejecutar la conversion es : ",os.getcwd())
			print("EL ARCHIVO pdf ES: ",_fileName3)
			print("EL ARCHIVO jpg ES: ",_fileName)
			...
			try:
				print("Se ha identificado que la imagen contiene alpha channel")
				print("Procediendo a Eliminar El Alpha Channel de la imagen")
				...
				print("el directorio en el que se va a ejecutar la conversion es : ",os.getcwd())
				print("EL ARCHIVO pdf ES: ",_SnALPHpdf)
				print("EL ARCHIVO jpg ES: ",_SnALPH)
				with open(_SnALPHpdf,"wb") as f:
					print(" el nombre del archivo antes de convertir a pdf es : ",_SnALPHpdf)
					f.write(img2pdf.convert(_SnALPH))
			except:
				print("se produjo un segundo error al convertir a PDF, se enviara el archivo en jpg")
				#_errorCONV=True
		finally:
...
#### BORRAR ESTE SEGMENTO <
	...
	def MoverPDF(self):
		print("se ha ingresado a la funsion Mover archivos")
